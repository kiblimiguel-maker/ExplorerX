...
export default function PlaceCard({ place, rank, userLocation, onLike, compact = false, featured = false, score, confirmFavoriteRemoval = false }: { place: Place; rank?: number; userLocation?: Coordinates | null; onLike?: () => void; compact?: boolean; featured?: boolean; score?: number; confirmFavoriteRemoval?: boolean }) {
  ...
  const transitionName = `place-image-${place.id.replace(/[^a-zA-Z0-9_-]/g, '-')}`
  const imageTransitionStyle = { viewTransitionName: transitionName } as CSSProperties
  return <article className={`place-card ${compact ? 'place-card-compact' : ''} ${featured ? 'place-card-featured' : ''}`}>
    <ViewTransitionLink to={`/places/${place.id}`} className="place-image-wrap" style={imageTransitionStyle}>
    ...
    <div className="place-card-body">
      <ViewTransitionLink to={`/places/${place.id}`}><h3>{place.name}</h3></ViewTransitionLink>
      ...
      <div className="place-meta"><span className={`category category-${place.category.toLowerCase()}`}><CategoryIcon category={place.category}/>{place.category}</span>{score !== undefined && <span className="score-label">{Math.round(score)} Punkte</span>}{onLike ? <button type="button" className="like-button" onClick={onLike} aria-label={`${place.name} liken`}><Heart size={18}/>{place.likes_count}</button> : <span className="like-count"><Heart size={16}/>{place.likes_count}</span>}</div>
    </div>
  </article>
}
