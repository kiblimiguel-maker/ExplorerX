...
export default function MapPage() {
  ...
  return <div className="map-page">
    <aside className={`map-sidebar map-sidebar-product ${sheetOpen ? 'sheet-open' : 'sheet-closed'}`}><button className="sheet-handle" type="button" onClick={() => setSheetOpen((current) => !current)} aria-label={sheetOpen ? 'Ergebnisse einklappen' : 'Ergebnisse öffnen'}><span/></button><div className="map-panel-top"><div><strong>Entdecken</strong><span>{filtered.length} echte Orte</span></div><div className="map-panel-actions"><button className="map-locate" onClick={locate} aria-label="Orte in deiner Nähe"><LocateFixed/></button><button className="sidebar-toggle" type="button" onClick={() => setSheetOpen((current) => !current)} aria-label={sheetOpen ? 'Seitenleiste einklappen' : 'Seitenleiste öffnen'}>{sheetOpen ? <PanelLeftClose/> : <PanelLeftOpen/>}</button></div></div><Filters query={query} onQuery={setQuery} category={category} onCategory={setCategory} features={features} onFeature={toggleFeature} places={places} compactCategories placeholder="Suche echte Orte…" quickSuggestions={[{ label: 'Trending', value: 'trending' }, { label: 'Sunset Spots', value: 'sunset' }, { label: 'Badestellen', value: 'baden' }, { label: 'Aussichtspunkte', value: 'aussicht' }]} onQuickSuggestion={quickFilter}/>{locationError && <p className="form-error" role="alert">{locationError}</p>}{isLoading ? <div className="loading-state">Orte werden geladen…</div> : filtered.length ? <MapDiscoveryRail places={filtered} location={location} onLike={toggleLike}/> : <div className="empty-state compact-empty"><h2>Noch nichts gefunden</h2><p>Entferne einen Filter oder suche nach einem neuen Abenteuer.</p></div>}</aside>
