...
function LoadedProfilePage() {
  ...
  return <div className="profile-page content-page">
    <section className="profile-hero"><div className="profile-cover" style={coverImage ? { backgroundImage: `url(${coverImage})` } : undefined}/><div className="profile-identity"><UserAvatar className="profile-avatar" url={preview || profile?.avatar_url} name={profile?.display_name || user.email} imageAlt="Profilbild"><label className="avatar-upload"><Camera/><span className="sr-only">Profilbild wählen</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => pickAvatar(event.target.files?.[0])}/></label></UserAvatar><div><div className="level-label"><Sparkles/> Explorer Level {level}</div><h1>{profile?.display_name || 'Neues Profil'}</h1><p>{profile?.bio || 'Erzähl der Community kurz, was du gern entdeckst.'}</p><div className="profile-xp" aria-label={`${levelProgress} von 100 Punkten bis zum nächsten Level`}><div><span>Level {level}</span><strong>{levelProgress} / 100 XP</strong></div><span><i style={{ width: `${levelProgress}%` }}/></span></div><div className="profile-actions">{isAdmin && <Link className="secondary-button" to="/admin"><ShieldCheck/> Adminbereich</Link>}<button className="secondary-button" type="button" onClick={logout}><LogOut/> Abmelden</button></div></div></div></section>
