...
function LoadedProfilePage() {
  ...
  return <div className="profile-page content-page">
    <section className="profile-hero profile-hero-clean"><div className="profile-cover" aria-hidden="true"/><div className="profile-identity"><div className="profile-summary"><div className="level-label"><Sparkles/> Explorer Level {level.level}</div><h1>{profile?.display_name || 'Neues Profil'}</h1><p>{profile?.bio || 'Erzähl der Community kurz, was du gern entdeckst.'}</p><div className="profile-xp" aria-label={`${level.progress} von ${level.target} XP bis Level ${level.nextLevel}`}><div><span>{stats.xp} XP gesamt</span><strong>Noch {level.remaining} XP bis Level {level.nextLevel}</strong></div><span><i style={{ width: `${levelProgress}%` }}/></span></div><div className="profile-actions">{isAdmin && <Link className="secondary-button" to="/admin"><ShieldCheck/> Adminbereich</Link>}<button className="secondary-button" type="button" onClick={logout}><LogOut/> Abmelden</button></div></div><div className="profile-avatar-block profile-avatar-block-clean"><button className="avatar-change-button" type="button" onClick={() => avatarInputRef.current?.click()} disabled={avatarUploading}>{avatarUploading ? <LoaderCircle className="spin"/> : <Camera/>}{avatarUploading ? 'Wird hochgeladen…' : 'Profilbild ändern'}</button><input ref={avatarInputRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" disabled={avatarUploading} onChange={avatarChanged}/>{avatarStatus && <small className="avatar-upload-status" role="status">{avatarStatus}</small>}</div></div></section>
